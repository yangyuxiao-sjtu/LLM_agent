import os
import json
file_path = "../prompts/prompts_n.json"
...
for sample in data:
    ...
    for sg in sample:
        ...
        if prompt == None:
            prompt ='Your task is: '+sg['task']+'\n'+ 'The objects that might help you to solve the task are:'+sg['predict'] +'\n'+sg['subgoal'].replace("HLA: ", "")+'\n'
        else:
            prompt+= 'The objects that might help you to solve the task are:'+sg['predict']+'\n'+sg['subgoal'].replace("HLA: ", "")+'\n'
    if(len(sample)<=4):
